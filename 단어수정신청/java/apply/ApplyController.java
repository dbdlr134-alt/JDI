	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		switch(command) {
		    ...
		    case "/reject.apply":
		        ...
		        //수정신청 입력처리
		    	case "/requestEdit.apply":

		        UserDTO user2 = (UserDTO)request.getSession().getAttribute("sessionUser");
		        if(user2 == null) {
		            response.sendRedirect("login.jsp");
		            return;
		        }
		        ...
		        ApplyDTO dto2 = new ApplyDTO();
		        dto2.setWordId(wordId);
		        dto2.setWord(request.getParameter("word"));
		        dto2.setDoc(request.getParameter("doc"));
		        dto2.setKorean(request.getParameter("korean"));
		        dto2.setJlpt(request.getParameter("jlpt"));
		        dto2.setJdiUser(user2.getJdi_user());

		        dao.insertEditApply(dto2);
		        ...
		        //수정신청 목록 조회(관리자)
		    	case "/adminEditList.apply":

		        UserDTO admin2 = (UserDTO)request.getSession().getAttribute("sessionUser");
		        if(admin2 == null || !"ADMIN".equals(admin2.getJdi_role())) {
		            response.sendRedirect("index.jsp");
		            return;
		        }

		        ArrayList<ApplyDTO> editList = dao.getEditWaitList();
		        request.setAttribute("list", editList);
		        request.getRequestDispatcher("admin/request_edit_list.jsp").forward(request, response);
		        break;
		        
		        //승인 처리
		    	case "/approveEdit.apply":

	    	    int editReqId = Integer.parseInt(request.getParameter("id"));
	    	    ApplyDTO editDto = dao.getEditApply(editReqId);

	    	    dao.approveEditApply(editDto);

	    	    response.sendRedirect("adminEditList.apply");
	    	    break;
		    	    
		    	 //거절 처리
		    	case "/rejectEdit.apply":
	    	    int rejectEditId = Integer.parseInt(request.getParameter("id"));
	    	    dao.updateStatus(rejectEditId, "NO");
	    	    response.sendRedirect("adminEditList.apply");
	    	    break;
